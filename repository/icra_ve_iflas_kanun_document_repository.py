...
class IcraVeIflasKanunDocumentRepository(DocumentRepositoryBase):
    def __init__(
            self,
            file_path: str
    ):
        ...
        self._logger.info(f"Initializing icra_ve_iflas_kanun Document Repository")
    ...
    async def aretrieve(
        ...
        docs = await self._db.asimilarity_search(query=hyde_generator_result, filter={"source": {"$eq": self.__class__.__name__}})
